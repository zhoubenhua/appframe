...
public class CommonUtil {
	 ...
	 * 检查是否有网络
	 * @param mContext  上下文
	 * @param noNetworkInfo 没有网络提示
	 * @return 是否有网络
	 */
	public static boolean checkNetwork(Context mContext,String noNetworkInfo) {
		ConnectivityManager cwjManager = (ConnectivityManager) mContext
				.getSystemService(Context.CONNECTIVITY_SERVICE);
		NetworkInfo info = cwjManager.getActiveNetworkInfo();
		boolean flag = info != null && info.isAvailable() ? true : false;
		if (!flag) {
			CommonUtil.sendToast(mContext,noNetworkInfo);
		}
		return flag;
	}
